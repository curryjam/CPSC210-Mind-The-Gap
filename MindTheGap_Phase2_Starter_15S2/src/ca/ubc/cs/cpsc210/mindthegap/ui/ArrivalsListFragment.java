...
public class ArrivalsListFragment extends ListFragment {
    ...
    public ArrayList<Arrival> getArrivalsForSelectedStationOnLineInDirection(String lineId, String travelDirn) {
        ...
        ArrayList<Arrival> ab = new ArrayList<>();
        Station selectedStation = StationManager.getInstance().getSelected();

        if (selectedStation != null) {
            for (ArrivalBoard nextAB : selectedStation) {
                ArrivalBoard selectedBoard;
                boolean foundSelected = false;

                if (!foundSelected) {
                    selectedBoard = nextAB;

                    if (selectedBoard.getLine().getId().equals(lineId) && selectedBoard.getTravelDirn().equals(travelDirn)) {
                        foundSelected = true;

                        for (Arrival nextA : selectedBoard) {
                            ab.add(nextA);
                        }
                    }
                }
